...
import * as fromUi from '../../../src/store/ui';
...
const loadResource = function* (action: Action) {
  const payload: fromDappsRenderer.LoadResourcePayload = action.payload;
  const tabs: Tab[] = yield select(fromTabs.getTabs);
  const settings: fromSettingsRenderer.Settings = yield select(fromSettings.getSettings);

  const namesBlockchain: undefined | Blockchain = yield select(fromSettingsRenderer.getNamesBlockchain);
  ...
  if (tabId) {
    if (!tab) {
      ...
      yield put(
        fromDappsRenderer.loadResourceFailedAction({
          tabId: tabId,
          url: payload.url,
          error: {
  ...
  */
  if (
    ...
    */
    ...
    yield put(
      fromDappsRenderer.createTabAction({
        tabId: tabId,
        resourceId: resourceId,
        url: payload.url,
      })
    );
    tabs = yield select(fromDappsRenderer.getTabs);
  ...
  yield put(
    fromDappsRenderer.focusAndActivateTabAction({
      tabId: tabId,
      resourceId: resourceId,
      url: payload.url,
    })
  ...
  */
  if (url.host.endsWith('.dappy')) {
    ...
    yield put(
      fromDappsRenderer.initTransitoryStateAndResetLoadErrorAction({
        tabId: tabId,
        resourceId: resourceId,
      })
    );
    ...
    if (dappAddress) {
      ...
      */
      ...
      try {
        ...
        yield put(fromDappsRenderer.loadResourceFailedAction({ tabId: tabId, url: payload.url, error: error }));
        return;
      }
      ...
      } = yield select(fromDappsRenderer.getLoadStates);
      ...
      */
      yield put(
        fromDappsRenderer.launchTabCompletedAction({
          tab: {
            ...tab,
            active: true,
...
export const loadResourceSaga = function* () {
  try {
    yield takeEvery(fromDappsRenderer.LOAD_RESOURCE, loadResource);
  } catch (err) {
    console.log(err);
  }
};
